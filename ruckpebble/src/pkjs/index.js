...
(function() {
  ...
  function openConfig(settingsSnapshot) {
    ...
    var compactHtml = '' +
      ...
      '<label>Body weight</label><input id="weight_value" type="number" step="0.1">' +
      '<label>Weight unit</label><select id="weight_unit"><option value="0">kg</option><option value="1">lb</option></select>' +
      '<label>Ruck weight unit</label><select id="ruck_weight_unit"><option value="0">kg</option><option value="1">lb</option></select>' +
      '<label>Stride length</label><input id="stride_length_value" type="number" step="0.1">' +
      '<label>Stride unit</label><select id="stride_length_unit"><option value="0">cm</option><option value="1">in</option></select>' +
      '</fieldset>' +
      '<fieldset><legend>Profile 1</legend>' +
      '<label>Name</label><input id="p1_name" type="text" maxlength="32">' +
      '<label class="il"><img src="data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill="#111" d="M5 5h6l1 9H4l1-9zm1-3h4v2H6V2zm1 1v1h2V3H7z"/></svg>') + '" alt="">Ruck weight</label><input id="p1_ruck_weight_value" type="number" step="0.1">' +
      ...
      '<label class="il"><img src="data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill="#111" d="M5 5h6l1 9H4l1-9zm1-3h4v2H6V2zm1 1v1h2V3H7z"/></svg>') + '" alt="">Ruck weight</label><input id="p2_ruck_weight_value" type="number" step="0.1">' +
      ...
      '<label class="il"><img src="data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill="#111" d="M5 5h6l1 9H4l1-9zm1-3h4v2H6V2zm1 1v1h2V3H7z"/></svg>') + '" alt="">Ruck weight</label><input id="p3_ruck_weight_value" type="number" step="0.1">' +
      ...
      'function draw(){var p=parseFloat($("chart_speed").value)||12,w=(parseFloat($("weight_value").value)||0),u=$("weight_unit").value==="1"?0.453592:1,ru=$("ruck_weight_unit").value,sp=60/p;function kg(i){var v=parseFloat($(i).value)||0;return ru==="1"?v*0.453592:v}function gr(i){return(parseInt($(i).value,10)||0)*10}function tf2(i){return tf($(i).value)}function wk(wk,sp){var sm=sp*1000/60;return(3.5+0.1*sm)*wk*60/200}function rk(wk,lk,sp,t,g){if(wk<=0)return 0;var V=sp/3.6,G=g/1000,T=t/100,tot=wk+lk,ratio=lk/wk,inner=1.5*V*V+0.35*V*G,mult=(1+Math.sqrt(0.3*V*V)/7+Math.pow(V*ratio,2)/4)*1.1,k=(1.5*wk+2*tot*ratio*ratio+T*tot*inner*mult)*3600/4184,walk=wk?wk:0;return lk>0&&k<walk?walk+walk*(lk/wk):k}var b=[["Walk",Math.round(wk(w*u,sp)),"#888"],[$("p1_name").value||"P1",Math.round(rk(w*u,kg("p1_ruck_weight_value"),sp,tf2("p1_terrain_type"),gr("p1_grade_percent"))),"#e45545"],[$("p2_name").value||"P2",Math.round(rk(w*u,kg("p2_ruck_weight_value"),sp,tf2("p2_terrain_type"),gr("p2_grade_percent"))),"#4a90d9"],[$("p3_name").value||"P3",Math.round(rk(w*u,kg("p3_ruck_weight_value"),sp,tf2("p3_terrain_type"),gr("p3_grade_percent"))),"#5cb85c"]],m=0;for(var i=0;i<b.length;i++)if(b[i][1]>m)m=b[i][1];var el=$("calorie_chart");el.innerHTML="";for(var j=0;j<b.length;j++){var row=ce("div",{class:"bar"}),lab=ce("div",{class:"lab"},b[j][0]),trk=ce("div",{class:"trk"}),fillEl=ce("div",{class:"fill"}),val=ce("span",{class:"val"},b[j][1]+" kcal");fillEl.style.width=(m?b[j][1]/m*100:0).toFixed(1)+"%";fillEl.style.background=b[j][2];trk.appendChild(fillEl);trk.appendChild(val);row.appendChild(lab);row.appendChild(trk);el.appendChild(row)}}' +
      'function fill(c){$("weight_value").value=(c.weight_value/10).toFixed(1);$("weight_unit").value=c.weight_unit;$("ruck_weight_unit").value=c.ruck_weight_unit;$("stride_length_value").value=(c.stride_length_value/10).toFixed(1);$("stride_length_unit").value=c.stride_length_unit;$("p1_name").value=c.profile1_name||"";$("p1_ruck_weight_value").value=(c.profile1_ruck_weight_value/10).toFixed(1);$("p1_terrain_type").value=tr(c.profile1_terrain_type,c.profile1_terrain_factor);$("p1_grade_percent").value=Math.round(c.profile1_grade_percent/10);$("p2_name").value=c.profile2_name||"";$("p2_ruck_weight_value").value=(c.profile2_ruck_weight_value/10).toFixed(1);$("p2_terrain_type").value=tr(c.profile2_terrain_type,c.profile2_terrain_factor);$("p2_grade_percent").value=Math.round(c.profile2_grade_percent/10);$("p3_name").value=c.profile3_name||"";$("p3_ruck_weight_value").value=(c.profile3_ruck_weight_value/10).toFixed(1);$("p3_terrain_type").value=tr(c.profile3_terrain_type,c.profile3_terrain_factor);$("p3_grade_percent").value=Math.round(c.profile3_grade_percent/10);draw()}' +
